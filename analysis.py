...
with col3:
    ...
    selected_quarters = st.multiselect(
        ...
        key="quarter_selector_multiselect"
    )
    if selected_quarters:
        df = df[df['Quarter'].isin(selected_quarters)]

# Business Line selector (allow multiple selections)
with col4:
    business_lines = sorted(df['Product'].dropna().unique())
    selected_business_lines = st.multiselect(
        "Select Business Line",
        options=business_lines,
        default=[business_lines[-1]],  # Default to the most recent business line
        key="business_line_selector_multiselect"
    )
...
if not df.empty:
    ...
    # Group data by "Claim Type" and calculate the average Claim Amount
    claim_type_avg = df.groupby('Claim Type')['Claim Amount'].mean().reset_index()
    ...
    with cols2:
        ...
        # Add trace for Average Claim Amount
        fig_claim_type_avg.add_trace(go.Bar(
            ...
            textfont=dict(color='white'),
            hoverinfo='x+y+name',
            marker_color=custom_colors[0]  # Use a single color for simplicity
        ))

        # Set layout for the chart
        fig_claim_type_avg.update_layout(
...
# Calculate the Approved Claim Amount by Client Segment
    int_owner = df.groupby("Claim Status")["Claim Amount"].sum().reset_index()
    int_owner.columns = ["Claim Status", "Claim Amount"]    

    with cls2:
        # Display the header
        st.markdown('<h3 class="custom-subheader">Total Claim Amount by Claim Status</h3>', unsafe_allow_html=True)


        # Create a donut chart
        fig = px.pie(int_owner, names="Claim Status", values="Claim Amount", hole=0.5, template="plotly_dark", color_discrete_sequence=custom_colors)
        fig.update_traces(textposition='inside', textinfo='value+percent')
        fig.update_layout(height=450, margin=dict(l=0, r=10, t=30, b=50))
